...
class inventory: 
    ...
    def show_all(self):
        if not self.product:
            ...
            for prod in self.product:
                print(f" produkt nummret är: {prod.product_nummber}, produkten heter: {prod.name} och den kostar {prod.price}kr ")

    def delete(self, product_nr):
        product_nr = int(product_nr)

        for prod  in self.product:
            if prod.product_nummber == product_nr :
    ...
    def uppdate_price(self, prod_nr, new_price):
        ...
        for produkter in self.product:
            if produkter.product_nummber ==  prod_nr:
